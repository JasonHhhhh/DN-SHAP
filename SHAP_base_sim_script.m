...
for i=1:length(cc0_start_iterp) % 四个压缩机 五个插值点
    ...
    upbound(i,:) = centor+interval*0;
    lowbound(i,:) = centor-interval*0;
end
...
k = 10;
[n_pr,~] = size(par.id_v);
doe = lhsdesign(1000,n_pr*n_control_tps);
doe = reshape(doe,1000,n_pr,n_control_tps);
finalv_doe = zeros(1000,n_pr,n_control_tps);
...
for i=1:3
    ...
    yi = zeros(5,25);
    for iii=1:5
        yi(iii,:)=interp1(xxx,cc0(iii,:),xx_new, 'linear');
    end
    cc0=yi;
%     f1=figure(1); clf
...
%     legend('Comp_2#','Comp_3#','Comp_4#','Comp_5#');
    ...
    [par]=tran_sim_setup_0(par_init,cc0);
    % execute simulation
...
SHAPtree_data_base.cost=cost;
SHAPtree_data_base.mass=mass;
SHAPtree_data_base.var=var;
SHAPtree_data_base.shap_cost=shap_cost;
SHAPtree_data_base.shap_mass=shap_mass;
SHAPtree_data_base.shap_var=shap_var;
SHAPtree_data_base.shap_error=shap_error;
SHAPtree_data_base.x_all=x_all;
