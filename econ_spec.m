...
if(par.intervals==0)
    %load input data on time series points 所有时间区间
    xd=double(dlmread([mfolder '\input_ts_tpts.csv'])); % 时间索引
    qbar=double(dlmread([mfolder '\input_ts_qbar.csv'])); % 通过用户分输量得到的出口流量 8个 其余都是0 但是所有非气源节点都要有29
    gbar=double(dlmread([mfolder '\input_ts_gbar.csv'])); % 所有用户分输流量 15个，对应节点求和则得到qbar,有几个分输节点就写几个
    dmax_0=double(dlmread([mfolder '\input_ts_dmax.csv'])); % 需求负荷边界
    smax_0=double(dlmread([mfolder '\input_ts_smax.csv'])); % 供气负荷边界
    cd_0=double(dlmread([mfolder '\input_ts_cd.csv'])); % 需求用气的价格
    cs_0=double(dlmread([mfolder '\input_ts_cs.csv'])); % 供货价格
    cslack=double(dlmread([mfolder '\input_ts_cslack.csv'])); % 气源供气价格
    pslack=double(dlmread([mfolder '\input_ts_pslack.csv'])); % 气源压力
...
if(par.intervals>0)
    %load input data on time intervals 指定的interval时间区间
...
par.m.Yq=[sparse(qbar)'/par.c.qsc;sparse(par.n.nv-par.n0.nv,length(xd))];%尽管分割，但是30个原负荷结点编号不变
par.m.Yubd=dmax'/par.c.qsc; % demand 和 supply 的流量约束
...
% averages of input functions % 每个s d 结点的流量约束求平均，在时间轴积分
...
%reinxing of optimized gnodes to flow nodes
