function [network_out]=gas_model_reconstruct_new_aslmax(network_in,lmax,disc_opt)
...
if(disc_opt==1)
    ...
    seg_div=floor(pipe_length(long_pipes)/lmax);  %how many segments to divide pipes into
elseif(disc_opt==0)
    ...
    seg_div=[floor(pipe_length(lpipes)/lmax);disc_seg(spipes)];
end
...
% 这里新建两个数组用来保存最新的
edges_cell = cell(length(long_pipes)+nc,1);
for i=1:nc
    edges_cell{i}=[from_id(end-nc+i),to_id(end-nc+i)];
end
for j=1:length(long_pipes)
    ...
    edges_cell{j+nc}=[from_id(long_pipes(j)); newnodes;to_id(long_pipes(j))];
...
%adjacency matrix
Amm=sparse(nv,ne); Amp=sparse(nv,ne); % 定义管道的进口和出口
for i=1:ne % 对于第i个管道 定义那个是进口 哪个是出口
    Amm(from_id(i),i)=-1; %作为进口则给-1
    Amp(to_id(i),i)=1; % 作为出口则给1
end
Am=Amp+Amm; % 求和，在第i列-1和1的行索引 表示 第i个管道的进出口
% for i=1:nv
%    Ad(i,:)=-(from_id==i)+(to_id==i);
% end
%slack and demand submatrices
As=Am(isslack==1,:);%提取气源结点对于管道的进出口属性
Ad=Am(isslack==0,:);%提取其他结点对于管道的进出口属性
