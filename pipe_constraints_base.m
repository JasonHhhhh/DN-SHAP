...
N=par.N; % 时间长度-1
Ts=par.Ts;
D=-par.D*2/Ts; %用来计算导数 时间长度*时间长度的网格
M=par.M; C=par.C; FN=par.FN; NE=par.NE; GN=par.GN;
w=par.w; 
...
Am=par.Am; Ad=par.Ad; As=par.As; 
d=par.d; %  66 *24时间长度
R=par.R; 
%variables
if(par.use_init_state==0)
    p=reshape(x(1:N1*FN),FN,N1); % 66 * 时长
    q=reshape(x(N1*FN+1:N1*M),NE,N1);
    comps=reshape(x(N1*M+1:N1*M+N1*C),C,N1); % 5*时长
    fd=reshape(x(N1*(M+C)+1:N1*(M+C+GN)),GN,N1); % 有变量中对30个Gnode的分配，N1个时间点 所有节点*时长
...
d(par.guniqueind,:)=d(par.guniqueind,:)+par.gtod*fd; % 最终是66个节点（分割后的个数）
qcomp=q(clinks,:);
