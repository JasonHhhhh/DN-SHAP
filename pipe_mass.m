function [p_mass]=pipe_mass(p,comps,par)
%index parameters
N1=par.N1; N=par.N; M=par.M; C=par.C; FN=par.FN; NE=par.NE; GN=par.GN;
Ts=par.Ts; w=par.w; D=-par.D*2/Ts; Lk=par.Lk;
...
p_mass(:,j)=Xs*diag(Lk)*(abs(Amj')*p(:,j))/2;
%   k(FN*(j-1)+1:FN*j)=...
