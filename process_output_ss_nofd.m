function [par]=process_output_ss_nofd(par)
...
if(par.out.units==1), 
    out.ss.dbase=out.ss.dbase/mmscfd_to_kgps; 
%     out.ss.gsub=out.ss.gsub/mmscfd_to_kgps; 
...
if(par.out.units==1), 
%     out.ss.trlmp=out.ss.trlmp*mmscfd_to_kgps; 
...
%         dlmwrite([mfolder '\output_ss_comp-hpmax-mp.csv'],double([[1:out.ss.n0.nc];out.ss.mult0_cmax(1,:)]),'precision',16,'delimiter',',');
    ...
    if(par.out.intervals_out>0)
        ...
        if(par.out.steadystateonly==0)
            ...
            dlmwrite([mfolder '\output_int_lmp-bidders.csv'],douint_flow_constble([out.ss.gunique';out.ss.dglmp_int]),'precision',16,'delimiter',',');
