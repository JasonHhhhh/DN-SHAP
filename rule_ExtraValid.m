...
% 先load数据
% 处理X dX
load('SHAPtree_data_0126.mat');
load('SHAPtree_data_ref1.mat');
load('SHAPtree_data_ref2.mat');
load('SHAPtree_data_ref3.mat');
...
for m=1:3
    for ls=1:4
        ...
        for i=1:24
            % 先研究2
            ctree = load(['data\model_mine\ctree_' num2str(i) '_m' num2str(m) ...
                '_ls' num2str(ls) '.mat']);
...
plot([2,4,6,8],ratio_ay(:,1).*100,'LineWidth',2); axis('tight'), 
xlabel('Number of classification levels'),ylabel('Boosting Percent(%)')
hold on
scatter([2,4,6,8],ratio_ay(:,1).*100);
title(['Comparison of metric ' name{1}],'fontweight','bold')

subaxis(1,3,2,'MarginLeft',0.1,'SpacingHoriz',0.1), 
plot([2,4,6,8],ratio_ay(:,2).*100,'LineWidth',2); axis('tight'), 
xlabel('Number of classification levels'),ylabel('Boosting Percent(%)')
hold on
scatter([2,4,6,8],ratio_ay(:,2).*100);
title(['Comparison of metric ' name{2}],'fontweight','bold')

subaxis(1,3,3,'MarginLeft',0.1,'SpacingHoriz',0.1), 
plot([2,4,6,8],ratio_ay(:,3).*100,'LineWidth',2); axis('tight'), 
xlabel('Number of classification levels'),ylabel('Boosting Percent(%)')
hold on
scatter([2,4,6,8],ratio_ay(:,3).*100);
